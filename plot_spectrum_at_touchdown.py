...
if __name__ == '__main__':
    ...
    color = iter(['salmon', 'maroon', 'navy', 'crimson', 'deeppink', 'purple', 'black', 'gold'])
    ...
    for file in spectrum_files:
        ...
        ax[0].plot(periods_at_touchdown, spectrum_at_touchdown, c=c, label=station, linewidth=1)
        ax[0].plot(periods_at_touchdown, average_spectrum, c=c, linestyle='--', linewidth=1)

        relative_spectrum = (10**spectrum_at_touchdown-10**average_spectrum) / 10**average_spectrum
        ax[1].plot(periods_at_touchdown, relative_spectrum, c=c, label=station, linewidth=1)
    ...
    ax[0].set_xlim([2, 10])
    ax[0].set_ylim([2, 8])
    ax[0].set_title('Spectrum at touchdown')
    ax[0].grid('minor')
    ax[0].legend()
