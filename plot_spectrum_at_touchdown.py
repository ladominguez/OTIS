...
spectrum_files  = glob.glob(os.path.join('spectra', 'daig','spectrum*' + component + '*.pkl'))
#spectrum_files += glob.glob(os.path.join('spectra', 'oxig','spectrum*' + component + '*.pkl'))
#spectrum_files += glob.glob(os.path.join('spectra', 'pzig','spectrum*' + component + '*.pkl'))
#spectrum_files += glob.glob(os.path.join('spectra', 'ppig','spectrum*' + component + '*.pkl'))
spectrum_files.sort()
...
if __name__ == '__main__':
    ...
    for file in spectrum_files:
        ...
        ax[0].plot(periods_at_touchdown, spectrum_at_touchdown, c=c, label=station.upper(), linewidth=1.5)
        ...
        ax[1].plot(periods_at_touchdown, relative_spectrum, c=c, label=station.upper(), linewidth=1.5)
        ax[1].fill_between(periods_at_touchdown, relative_spectrum, 0, color=c, alpha=0.2)

    #ax.set_xscale('log')
    #ax.set_yscale('log')
    ax[0].set_xlabel('Period (s)')
    ax[0].set_ylabel(r'Amplitude $[m^2/s^4/Hz] [dB]$')
    ax[0].set_xlim([2, 10])
    #ax[0].set_ylim([2, 10])
    ...
    ax[1].set_xlim([2, 10])
    ax[1].set_ylim([0, 180])
    filename_out = f'spectrum_at_touchdown_{component}_PPIG.png'
    fig.savefig(filename_out,dpi=500,bbox_inches='tight')
    print(filename_out)
    fig.show()
